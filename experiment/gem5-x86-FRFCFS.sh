...
if [ "$BENCHMARK" == "parsec" ] ; then
	...
	/home/abj456/gem5/build/X86-FRFCFS/gem5.fast \
		-d ./parsec-gem5/m5out-${SIZE}-h${HEAVY_TAIL}-l${LIGHT_TAIL}/${STATS_DIR}/frfcfs/ \
	...
	KVM_TICKS=$6
	MEM_MODEL=$7
	if [ -z "$KVM_TICKS" ] || [ -z "$MEM_MODEL" ]; then
		KVM_TICKS="0.5e12"
		MEM_MODEL="DDR5_4400"
	fi
	if [ -z "$SIZE" ] || [ -z "$HEAVY_TAIL" ] || [ -z "$LIGHT_TAIL" ] || [ -z "$STATS_DIR" ]; then
	...
	/home/abj456/gem5/build/X86-FRFCFS/gem5.fast \
		...
		--size=$SIZE --heavy_threads=$HEAVY_TAIL --light_threads=$LIGHT_TAIL \
		--kvm_ticks=$KVM_TICKS --mem-model=$MEM_MODEL
