...
parser.add_argument(
    ...
    type=str,
    default="0.5e12",
    help="Number of ticks to run the Timing simulation before exiting.",
)
...
timing_sim_ticks = int(float(args.timing_ticks))

command = (
    # f"./remixed-parsec-workload.sh {test_size} {args.heavy_threads} {args.light_threads};"
    f"./run-20core-parsec.sh {test_size};"
    + f"sleep {sleep_time};"
    + "m5 exit;"
)
...
def exit_event_handler():
    ...
    m5.scheduleTickExitFromCurrent(kvm_sim_ticks + timing_sim_ticks)
    print("Scheduling exit event for KVM ticks:", args.kvm_ticks)
    print("Scheduling exit event for Timing ticks:", args.timing_ticks)
...
def scheduled_tick_event_handler():
    ...
    print("Scheduled tick event handler SECOND called")
